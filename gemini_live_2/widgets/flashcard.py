"""Server-side contract for the two-sided flashcard widget."""

from __future__ import annotations

from typing import Any, Mapping

from .registry import StageMapPolicy, StageMapTextSource, StageMapView, WidgetAnchor, WidgetAssetReferenceDefinition, WidgetDefinition, WidgetInteractionDefinition, WidgetPropDefinition, WidgetPropsError, WidgetStateDefinition


def _text(value: object, field_name: str) -> str:
    if not isinstance(value, str) or not value.strip(): raise WidgetPropsError(f"{field_name} must be a non-empty string.")
    return value.strip()
...
def _validate_props(props: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if asset_id is not None:
        ...
        normalized_front["remote_image_result_id"] = _text(
            remote_image_result_id,
            "flashcard.front.remote_image_result_id",
        )
    result = {
...
def definition(*, visibility_state: WidgetStateDefinition, flipped_state: WidgetStateDefinition) -> WidgetDefinition:
    return WidgetDefinition(
        widget_id="flashcard", validate_props=_validate_props,
        ...
        props=(
            WidgetPropDefinition(
                ...
                "object",
                True,
                "Mặt trước gồm một ảnh và chữ ngắn thật sự hiển thị.",
                template_value_kind="binding",
                child_props=(
                    WidgetPropDefinition(
                        "asset_id", "string", False, "ID ảnh từ Asset Catalog.",
                        source="asset_catalog.id", template_value_kind="binding",
                    ),
                    WidgetPropDefinition(
                        "remote_image_result_id", "string", False,
                        "ID ảnh từ kết quả search_image của session hiện tại.",
                        source="search_image.result_id", template_value_kind="binding",
                    ),
                    WidgetPropDefinition(
                        "text", "string", True,
                        "Chữ thật sự hiển thị ở mặt trước.",
                        template_value_kind="binding",
                    ),
                ),
            ...
            WidgetPropDefinition(
                "back", "object", True,
                "Mặt sau gồm word, phonetic và meaning đều hiển thị.",
                template_value_kind="binding",
                child_props=(
        ...
        stage_map_policy=StageMapPolicy(kind="flashcard", anchor_key="card", views=(
        ...
        asset_references=(
            WidgetAssetReferenceDefinition(path="props.front.asset_id", required=False),
        ),
    )
