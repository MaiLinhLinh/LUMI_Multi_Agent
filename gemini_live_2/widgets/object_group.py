"""Server-side contract for the object_group widget."""
...
def _positive(value: object, field_name: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 1: raise WidgetPropsError(f"{field_name} must be a positive integer.")
    return value
...
def _anchors(props: Mapping[str, Any]) -> tuple[WidgetAnchor, ...]:
    return (WidgetAnchor("group", ("highlight", "circle")), *(WidgetAnchor(f"item_{index}", ("highlight", "circle")) for index in range(1, props["count"] + 1)))


def definition(*, visibility_state: WidgetStateDefinition) -> WidgetDefinition:
    return WidgetDefinition(
        widget_id="object_group", validate_props=_validate_props, anchor_policy=_anchors,
