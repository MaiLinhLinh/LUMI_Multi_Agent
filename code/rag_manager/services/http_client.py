...
def get_json(
    ...
    *,
    ...
    try:
        ...
        return _error(source, f"HTTP error {status_code}.", status_code)
    except httpx.RequestError as exc:
        return _error(source, f"Network error: {exc}", None)
    except ValueError:
        return _error(source, "Response was not valid JSON.", response.status_code)

    if not isinstance(data, dict):
        return _error(source, "Response JSON was not an object.", response.status_code)
    return {"ok": True, "data": data}


def _error(source: str, message: str, status_code: int | None) -> ServiceResponse:
    return {
        "ok": False,
        "error": {
            "source": source,
