...
class MusicSearchService:
    ...
    def find_exact_track(
        ...
        *,
        ...
        """Return title-and-artist matches from the in-memory catalog, without embedding."""

        requested_title = normalize_music_text(title)
        requested_artist = normalize_music_text(artist)
        if not requested_title or not requested_artist:
            return []
        limit = top_k or self.output_top_k
        matches = [
            record
        ...
        return [
            ...
            for record in _sort_catalog_records(matches)[:limit]
        ]
    ...
    def find_by_artist(
        ...
        *,
        ...
        """Return verified same-artist catalog tracks, without an embedding request."""
        ...
        limit = top_k or self.output_top_k
        matches = [
            record for record in self._records if _artist_matches(record, requested_artist)
        ]
        return [
            _candidate_payload(record, final_score=None)
            for record in _sort_catalog_records(matches)[:limit]
...
def _title_matches(record: MusicSearchRecord, requested_title: str) -> bool:
    aliases = music_title_aliases(
        str(record.metadata.get("title", "")),
        record.metadata.get("title_aliases")
        if isinstance(record.metadata.get("title_aliases"), list)
        else None,
    )
...
def _artist_matches(record: MusicSearchRecord, requested_artist: str) -> bool:
    return any(
        _tokens_compatible(requested_artist, normalize_music_text(str(candidate)))
        for candidate in record.metadata.get("artist_keys", [])
    )


def _sort_catalog_records(records: Sequence[MusicSearchRecord]) -> list[MusicSearchRecord]:
    return sorted(
        records,
        key=lambda record: (int(record.metadata.get("view_count", 0)), record.id),
        reverse=True,
    )
